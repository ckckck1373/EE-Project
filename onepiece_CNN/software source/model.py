...
class ResBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if(quantization==0):
            ...
            #printdata(res*256, "RES1_2_golden", False)
...
class onepieceSRNet(nn.Module):
    ...
    def forward(self, x):
        ...
        if(quantization==0):
            ...
            if(plot_enable):
                plt_his("up1_out", f_x, 10)
            #printdata(f_x*256, "UP_1", False)
            f_x = self.upsamp2(f_x)
            if(plot_enable):
                plt_his("up2_out", f_x, 11)
            # printdata(f_x*256, "UP_2", False)
            output = self.conv2(f_x)
            ...
            #output=output*2
            # if(plot_enable):
            #     plt_his("output_before_quantize", output, 13)

            #output = 2*output # return one place back
            if(plot_enable):
                plt_his("out", output, 12)
            #printdata(output*256, "conv2_golden", False)
            return output
...
def printdata(temp_output_data, filename, en_relu): 
    ...
    elif((filename=="UP_2") or (filename=="conv2_golden")):
                            ...
                            for ch in range(24): 
                                for j in range(2):
                                    for i in range(2):
                                        ...
                                        if(filename=="conv1_input" or filename=="conv2_golden"):
                                            if(ch<3):
